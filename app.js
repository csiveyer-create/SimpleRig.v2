...
function newProject(){if(!confirm('Create a new project?'))return;Object.assign(state,{projectId:uid(),projectName:'Untitled Project',objects:[],lines:[],background:null,backgroundOpacity:.8,layers:JSON.parse(JSON.stringify(layers)),selectedId:null,history:[],future:[],drawings:[],calendar:[],harnessNotes:'',sequences:[],currentSequenceId:null,currentRigId:null,harnessItems:[],harnessSelectedId:null,showObjectLabels:false,additionalLineTensionKg:0});$('projectName').value=state.projectName;$('harnessNotes').value='';loadBackground();renderAll();saveProject()}
...
function renderAnalysisSummary(){const el=$('analysisSummary');if(!el)return;if($('additionalLineTensionKg'))$('additionalLineTensionKg').value=state.additionalLineTensionKg??0;if(!state.analysisVisible||!state.analysis){el.className='analysis-summary hint';el.innerHTML='Connect a load, pulleys and anchors with rig lines, then select <strong>Analyse Rig</strong>.';return}const a=state.analysis;el.className='analysis-summary';el.innerHTML=`<div class="analysis-card"><span>Load</span><strong>${a.loadMassKg.toFixed(1)} kg</strong></div><div class="analysis-card"><span>Ideal mechanical advantage</span><strong>${a.idealMA.toFixed(2)} : 1</strong></div><div class="analysis-card"><span>Estimated effective advantage</span><strong>${a.effectiveMA.toFixed(2)} : 1</strong></div><div class="analysis-card"><span>Estimated pull from load</span><strong>${a.pullKg.toFixed(1)} kgf</strong></div><div class="analysis-card"><span>Additional line tension</span><strong>${a.additionalLineTensionKg.toFixed(1)} kgf</strong></div><div class="analysis-card"><span>Total line tension</span><strong>${a.tensionKg.toFixed(1)} kgf</strong></div><div class="analysis-card"><span>Pulleys</span><strong>${a.movingPulleyCount} moving · ${a.fixedPulleyCount} fixed</strong></div>${a.warnings.map(w=>`<div class="analysis-card analysis-warning">${w}</div>`).join('')}<div class="analysis-card analysis-warning">Planning estimate. Additional line tension is added to the calculated pull and increases the calculated load on connected rope segments, pulleys, shackles, hardware and anchors. Verify pulley roles, friction, angles, WLLs and dynamic effects independently.</div>`}
