...
function img(src,cors=false){
if(imageCache.has(src))return imageCache.get(src);
const i=new Image();
if(cors)i.crossOrigin='anonymous';
i.onload=draw;
i.onerror=()=>{console.error('SimpleRig asset failed to load:',src);draw()};
i.src=/^(data:|https?:|blob:)/.test(src)?src:encodeURI(src);
...
for(const category of assetCategories){
  ...
  for(const sourceAsset of matching){ const a={...sourceAsset,categoryKey:category.key,categoryTitle:category.title};
    ...
    d.innerHTML=`<img src="${encodeURI(a.src)}" alt="${a.name}"><span>${a.name}</span>`;
    const preview=d.querySelector('img');
    preview.addEventListener('error',()=>{
      preview.style.display='none';
      d.classList.add('asset-load-error');
      d.title='Asset file could not be loaded: '+a.src;
    });
