...
function renderAll(){draw();renderInspector();renderLayers();renderHierarchy();renderDrawingLibrary();renderCalendar();renderProjects();drawHarness()}
...
['objName','objWidth','objHeight','objRotation','objOpacity','objColor','objNotes','noteFontSize','noteTextColor','noteBackgroundColor','noteBackgroundToggle'].forEach(id=>$(id).onchange=updateInspector);
...
$('exportPngBtn').onclick=()=>{state.selectedId=null;draw();try{canvas.toBlob(b=>download(b,(state.projectName||'SimpleRig')+'.png'))}catch{alert('A remote image blocks export. Download and re-import it locally.')}};$('saveDrawingBtn').onclick=saveDrawing;
