...
function newProject(){if(!confirm('Create a new project?'))return;Object.assign(state,{projectId:uid(),projectName:'Untitled Project',objects:[],lines:[],background:null,backgroundOpacity:.8,layers:JSON.parse(JSON.stringify(layers)),selectedId:null,history:[],future:[],drawings:[],calendar:[],harnessNotes:'',sequences:[],currentSequenceId:null,currentRigId:null,harnessItems:[],harnessSelectedId:null,showObjectLabels:false});$('projectName').value=state.projectName;$('harnessNotes').value='';loadBackground();renderAll();saveProject()}
...
function renderAll(){draw();renderInspector();renderLayers();renderAnalysisSummary();renderHierarchy();renderDrawingLibrary();renderCalendar();renderProjects();drawHarness()}
...
$('exportPngBtn').onclick=()=>{state.selectedId=null;draw();try{canvas.toBlob(b=>download(b,(state.projectName||'SimpleRig')+'.png'))}catch{alert('A remote image blocks export. Download and re-import it locally.')}};$('saveDrawingBtn').onclick=saveDrawing;$('analyseRigBtn').onclick=analyseRig;$('clearAnalysisBtn').onclick=clearAnalysis;
